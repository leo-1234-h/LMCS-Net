...
class C2f(nn.Module):
    ...
    def forward(self, x):
        ...
        y.extend(m(y[-1]) for m in self.m)
        # print("C2f_Size:",self.cv2(torch.cat(y, 1)).shape)
        return self.cv2(torch.cat(y, 1))
...
class CIB(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # print("CIB_size:",self.cv1(x).shape)
...
class SCDown(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # print("SCDown:",self.cv2(self.cv1(x)).shape)
...
class Bottleneck_FDDM(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3)):
        super().__init__()
        # 第一阶段：压缩到 c_
        self.cv1 = Conv(c1, c1//2, k[0], 1)
        # 第二阶段：低秩调制，保持 c_ 不变
        self.cv2 = FDDM(c1//2)
        # 第三阶段：把通道数投影回 c2
        self.proj = Conv(c1//2, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        print("enter:")
        z = self.cv1(x)          # [N, c_, H, W]
        # print("cv1:",z.shape)
        z = self.cv2(z)          # [N, c_, H, W]
        # print("cv2:", z.shape)
        z = self.proj(z)         # [N, c2, H, W]
        # print("cv3:", z.shape)
