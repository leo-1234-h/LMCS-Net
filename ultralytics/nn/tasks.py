...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    #     elif m is TopMixConv:
    #         # 输出通道按官方规则做 width/max_channels 缩放
    #      pass
    #     elif m in frozenset(
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # --- prepare helper for flexible `from` field (support int or list) ---
        f_list = f if isinstance(f, (list, tuple)) else [f]
        # chs: list of channel numbers for each source in f_list
        # Protect indexing: if someone used invalid f index, raise clearer error
        try:
            chs = [ch[x] for x in f_list]
        except Exception as e:
            raise IndexError(f"parse_model: invalid 'from' index {f_list} at layer {i}") from e

        # helper: single-channel value when a module expects a single input.
        single_ch = chs[0] if len(chs) == 1 else sum(chs)

        # -------------------------
        # EARLY special-case: TopMixConv
        # -> ensure args are exactly what TopMixConv.__init__ expects,
        #    before any other branch mutates args.
        # -------------------------
        if m is TopMixConv:
            # yaml may supply partial args like [out_c] or [out_c, k, groups, S, sd]
            raw = list(args) if isinstance(args, (list, tuple)) else [args]
            out_c = raw[0] if len(raw) > 0 else single_ch
            k = raw[1] if len(raw) > 1 else 3
            g = raw[2] if len(raw) > 2 else 4
            S = raw[3] if len(raw) > 3 else 7
            sd = raw[4] if len(raw) > 4 else 1

            # follow Ultralytics scaling for out channels
            if out_c != nc:
                out_c = make_divisible(min(out_c, max_channels) * width, 8)

            # overwrite args so m(*args) receives exactly these five values
            args = [out_c, k, g, S, sd]
            c2 = out_c

            # debug info (optional): prints to logger if verbose
            if verbose:
                LOGGER.info(f"TopMixConv parsed at layer {i}: from={f_list} args={args}")

        # ---------- original dispatch logic ----------
        elif m in base_modules:
            c1, c2 = single_ch, args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            if m is C2fAttn:  # set 1) embed channels and 2) num heads
                args[1] = make_divisible(min(args[1], max_channels // 2) * width, 8)
            ...
            c1, cm, c2 = single_ch, args[0], args[1]
            args = [c1, cm, c2, *args[2:]]
            if m is HGBlock:
                args.insert(4, n)  # number of repeats
                n = 1
            ...
            args.insert(1, [ch[x] for x in f_list])
        ...
        # construct module (repeat n times if needed)
        # debug: show module and args length when verbose (helps find mismatches)
        # if verbose:
        #     try:
        #         LOGGER.info(
        #             f"Building layer {i}: module={m.__name__ if hasattr(m, '__name__') else str(m)} from={f} args={args}")
        #     except Exception:
        #         LOGGER.info(f"Building layer {i}: module={str(m)} from={f} args={args}")
